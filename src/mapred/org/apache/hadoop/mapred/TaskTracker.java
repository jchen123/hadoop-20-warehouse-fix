  ...
  */
  ...
  public TaskTracker(JobConf conf) throws IOException {
    ...
    Class<? extends ResourceCalculatorPlugin> clazz =
        conf.getClass(MAPRED_TASKTRACKER_MEMORY_CALCULATOR_PLUGIN_PROPERTY,
    ...
    maxMapSlots = getMaxSlots(conf, numCpuOnTT, TaskType.MAP);
    maxReduceSlots = getMaxSlots(conf, numCpuOnTT, TaskType.REDUCE);
  ...
  private boolean doCheckBuildVersion() {
  ...
  class TaskInProgress {
    ...
    public synchronized void reportDone() {
      ...
      // Checking the existence of map output directory
      if (task.isMapTask()) {
        LocalDirAllocator lDirAlloc = TaskTracker.this.localDirAllocator;
        String jobId = task.getJobID().toString();
        String taskId = task.getTaskID().toString();
        try {
          String outputDir =
            TaskTracker.getIntermediateOutputDir(jobId, taskId);
          Path mapOutput = lDirAlloc.getLocalPathToRead(
              outputDir + "/file.out", TaskTracker.this.fConf);
          Path mapOutputIndex = lDirAlloc.getLocalPathToRead(
              outputDir + "/file.out.index", TaskTracker.this.fConf);
          LOG.info("Map output for " + taskId + " is in " + mapOutput);
          LOG.info("Map output index for " + taskId +
                   " is in " + mapOutputIndex);
        } catch (IOException e) {
          LOG.warn("Cannot find map output for " + taskId, e);
        }
      }
    }
   ...
   */
  private int getMaxSlots(JobConf conf, int numCpuOnTT, TaskType type) {
    int maxSlots;
    String cpuToSlots;
