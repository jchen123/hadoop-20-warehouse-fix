...
class FSDirectory implements FSConstants, Closeable {
   ...
   */
  INode delete(String src, List<Block> collectedBlocks) {
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug("DIR* FSDirectory.delete: "+src);
    }
    waitForReady();
    long now = FSNamesystem.now();
    INode deletedNode = unprotectedDelete(src, collectedBlocks, now);
    if (deletedNode != null) {
  ...
  INode unprotectedDelete(String src, long modificationTime) {
    return unprotectedDelete(src, null, modificationTime);
  }
   ...
   * @return the deleted target inode, null if deletion failed
   */ 
  INode unprotectedDelete(String src, List<Block> toBeDeletedBlocks,
                          long modificationTime) {
    src = normalizePath(src);
      ...
      if (targetNode == null) { // non-existent src
        ...
        try {
          ...
          if (toBeDeletedBlocks == null) {
            ...
            namesystem.removePathAndBlocks(src, null);
          }
          if (NameNode.stateChangeLog.isDebugEnabled()) {
            NameNode.stateChangeLog.debug("DIR* FSDirectory.unprotectedDelete: "
                +src+" is removed");
   ...
   */
  DirectoryListing getPartialListing(String src, byte[] startAfter,
   ...
   * Create HdfsFileStatus by file INode 
   */
   private HdfsFileStatus createHdfsFileStatus(byte[] path, INode node) {
